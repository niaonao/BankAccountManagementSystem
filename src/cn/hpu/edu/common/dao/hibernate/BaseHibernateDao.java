...
import cn.hpu.edu.common.util.NumberUtil;
...
public abstract class BaseHibernateDao extends HibernateDaoSupport implements BaseDao {
	...
	/* (non-Javadoc)
	 * 批量删除
	 * @see cn.hpu.edu.common.dao.hibernate.BaseDao#deleteByIds(java.lang.Class, java.lang.String)
	 */
	...
	protected Page queryByPage(final String listHql, final List<? extends Object> params, final Integer firstResult, final Integer maxResults,
		...
		if (totalCount > 0) {
			...
			//select count(*) from (select account0_.id as id0_, account0_.accountType as accountT2_0_, account0_.balance as balance0_, account0_.idcard as idcard0_, account0_.interest as interest0_, account0_.password as password0_, account0_.phone as phone0_, account0_.realname as realname0_, account0_.sid as sid0_, account0_.status as status0_, account0_.useType as useType0_ from t_account account0_ where 1=1 and (account0_.idcard like ?)) tmp_count_table
			if (params != null && !params.isEmpty()) {
				int size = params.size();
				for (int i = 0; i < size; i++) {
